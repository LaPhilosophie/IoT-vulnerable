...
The Tenda FH1205 V2.0.0.7(775) firmware has a stack overflow vulnerability in the `formQuickIndex` function. The `v5` variable receives the `PPPOEPassword` parameter from a POST request and is later assigned to the `v7` variable, which is fixed at 72 bytes. However, since the user can control the input of  `PPPOEPassword`, in function `decodePwd`, the statement: 
...
![image-20240320105336214](https://raw.githubusercontent.com/abcdefg-png/images/main/image-20240320105336214.png)