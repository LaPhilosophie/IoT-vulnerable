...
In the D-Link DIR-619L B1 2.06 firmware has a buffer overflow vulnerability in the `formEasySetupWWConfig` function. The `Var` variable receives the `curTime` parameter from a POST request. However, since the user can control the input of `curTime`, the `sprintf` can cause a buffer overflow vulnerability.
