...
In the D-Link DIR-605L 2.13B01 BETA firmware has a buffer overflow vulnerability in the `formSetEasy_Wizard` function. The `Var` variable receives the `curTime` parameter from a POST request. However, since the user can control the input of `curTime`, the `sprintf` can cause a buffer overflow vulnerability.

![image-20240926122027336](https://raw.githubusercontent.com/abcdefg-png/images2/main/image-20240926122027336.png)
...
def poc():
    target_url = "goform/formSetEasy_Wizard"
