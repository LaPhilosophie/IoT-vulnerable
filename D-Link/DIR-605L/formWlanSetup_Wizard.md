...
In the D-Link DIR-605L 2.13B01 BETA firmware has a buffer overflow vulnerability in the `formWlanSetup_Wizard` function. The `Var` variable receives the `webpage` parameter from a POST request. However, since the user can control the input of `webpage`, the `strcpy` can cause a buffer overflow vulnerability.

![image-20240926123133253](https://raw.githubusercontent.com/abcdefg-png/images2/main/image-20240926123133253.png)
...
def poc():
    target_url = "goform/formWlanSetup_Wizard"
