...
In the D-Link DIR-605L 2.13B01 BETA firmware has a buffer overflow vulnerability in the `formSetWanNonLogin` function. The `v4` variable receives the `webpage` parameter from a POST request. However, since the user can control the input of `webpage`, the `strcpy` can cause a buffer overflow vulnerability.
...
def poc():
    target_url = "goform/formSetWanNonLogin"
