...
- Firmware download website: https://www.tp-link.com/en/support/download/tl-wr941nd/
...
In the TL-WR941ND V6 Firmware has a stack overflow vulnerability in the `/userRpm/popupSiteSurveyRpm.htm` url. The `v21` variable receives the `ssid` parameter from a POST request and is later assigned to the `v53` variable, which is fixed at 17 bytes. 
